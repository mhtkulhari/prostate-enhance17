...
def differentiable_boundary(mask, radius=2):
    if mask.dim() == 3:
...
class BinaryBoundaryLoss(nn.Module):
    def __init__(self, radius=2):
        super(BinaryBoundaryLoss, self).__init__()
        self.radius = radius
    ...
    def forward(self, logits, target, mask=None):
        ...
        target_boundary = differentiable_boundary(target.float(), self.radius)
        pred_boundary = differentiable_boundary(prob, self.radius)
        if mask is not None:
            if mask.dim() == 3:
                mask = mask.unsqueeze(1)
            target_boundary = target_boundary * mask.float()
            pred_boundary = pred_boundary * mask.float()
        smooth = 1e-5
        intersect = torch.sum(pred_boundary * target_boundary)
        denom = torch.sum(pred_boundary * pred_boundary) + torch.sum(target_boundary * target_boundary)
